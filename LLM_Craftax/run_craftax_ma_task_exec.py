...
from craftax_agents import ExecAgent
from prompts import get_task_selection_system_prompt
from tree_search import MonteCarloTreeSearch
# from mcts_test_prompts import 
from mcts_prompts_code import get_alice_prompt, get_bob_prompt
...
SERVER_IP = 'http://172.31.69.37:11434/v1'
...
def generate_joint_policy(obs, config):

    mcts = MonteCarloTreeSearch(
        model=config.model_id,
        api_key= OPENAI_API_KEY if config.remote_api else "ollama",
        base_url= SERVER_IP if not config.remote_api else ""
    )

    run_path = config.run_path

    shutil.copy(get_original_cwd() + "/joint_policy_template.py", run_path + "/joint_policy.py")

    question = get_alice_prompt(config.objective, obs["agent_0"]["short_term_context"] + "\n" + obs["agent_0"]["long_term_context"])  # Get Alice's prompt for the task
    reflection_prompt = get_bob_prompt(obs["agent_1"]["short_term_context"] + "\n" + obs["agent_1"]["long_term_context"])  # Get Bob's prompt for reflecting on Alice's plan 
    mcts_plans_list, message_list = mcts.run(question, reflection_prompt)
    mcts_final_plan = mcts_plans_list[-1]
    ...
    try:
        match = re.search(r"Action plan\s*:\s*```python\s*(.*?)```", mcts_final_plan, re.DOTALL)
    except Exception as e:
        print(f"Error during regex search: {e}")
        match = None

    if match:
        joint_policy_func = match.group(1).strip()

    source = open(run_path + "/joint_policy.py").read()

    result = insert_method_into_class(source, "CraftaxJointPolicy", joint_policy_func)
    open(run_path + "/joint_policy.py", "w").write(result)
    
    return mcts_plans_list, message_list


def run_task_execution(env, agents, config):
    ...
    obs_history = {}
    cum_rewards = {agent_name: 0 for agent_name in env.agents}

    for agent_name in env.env.agents:
        agents[agent_name].reset()
        obs_history[agent_names[agent_name]] = []
    episode_dict = {}
    actions_dict = {}
    # final_plan = None
    ...
    mcts_plans_list, message_list = generate_joint_policy(obs, config)
    JointPolicy = load_joint_policy(config.run_path)
    joint_policy = JointPolicy("Alice and Bob")
    ...
    for step in range(config.max_episode_steps):
        ...
        for ind, agent_name in enumerate(env.agents):
            actions_dict[agent_name] = None  # Initialize with None to indicate no action taken yet
            
            obs_agent = obs[agent_name]["short_term_context"] + "\n" + obs[agent_name]["long_term_context"]

            obs_history[agent_names[agent_name]].append(obs_agent)
            ...
            if agents[agent_name].current_objective is None:
                new_objective = joint_policy.action_plan(agent_names[agent_name], obs_history)
                agents[agent_name].current_objective = new_objective
                episode_dict[step][agent_name]['objective'] = new_objective
            else:
                if agents[agent_name].current_plan is None:
                    new_objective = joint_policy.action_plan(agent_names[agent_name], obs_history)
                    agents[agent_name].current_objective = new_objective
                    episode_dict[step][agent_name]['objective'] = new_objective
                else:
                    episode_dict[step][agent_name]['objective'] = agents[agent_name].current_objective
        ...
        if config.parallel_execution:
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        if response is not None:
                            episode_dict[step][agent_name]['plan'] = response["plan"]
                            episode_dict[step][agent_name]['reasoning'] = response["reasoning"]
                        ...
                        #     episode_dict[step][agent_name]['objective_completion'] = response["objective_completion"]
                        else:
                            episode_dict[step][agent_name]['plan'] = episode_dict[step - 1][agent_name]['plan']
                            episode_dict[step][agent_name]['reasoning'] = episode_dict[step - 1][agent_name]['reasoning']
                        ...
                        #     episode_dict[step][agent_name]['objective_completion'] = episode_dict[step - 1][agent_name]['objective_completion']
                    except Exception as e:
            ...
            for agent_name in env.env.agents:
                ...
                #     else:
                #         episode_dict[step][agent_name]['objective_reasoning'] = None
                ...
                #     else:
                #         episode_dict[step][agent_name]['plan_reasoning'] = None
                #     episode_dict[step][agent_name]['objective_completion'] = response["objective_completion"]
                ...
                #     # print("Plan :", episode_dict[step][agent_name]['plan'])
                    ...
                    episode_dict[step][agent_name]['plan'] = episode_dict[step - 1][agent_name]['plan']
                    episode_dict[step][agent_name]['reasoning'] = episode_dict[step - 1][agent_name]['reasoning']
                #     if "objective" in episode_dict[step - 1][agent_name].keys():
                #         episode_dict[step][agent_name]['objective'] = episode_dict[step - 1][agent_name]['objective']
                #         episode_dict[step][agent_name]['plan'] = episode_dict[step - 1][agent_name]['plan']
                #         episode_dict[step][agent_name]['objective_reasoning'] = episode_dict[step - 1][agent_name]['objective_reasoning']
                #         episode_dict[step][agent_name]['plan_reasoning'] = episode_dict[step - 1][agent_name]['plan_reasoning']
                #         episode_dict[step][agent_name]['objective_completion'] = episode_dict[step - 1][agent_name]['objective_completion']
                #     else:
                #         episode_dict[step][agent_name]['objective'] = agent_objectives[agent_name]
                #         episode_dict[step][agent_name]['plan'] = None
                #         episode_dict[step][agent_name]['objective_reasoning'] = None
                #         episode_dict[step][agent_name]['plan_reasoning'] = None
                #         episode_dict[step][agent_name]['objective_completion'] = None
        ...
        cum_rewards = {agent_name: cum_rewards[agent_name] + rewards[agent_name] for agent_name in env.env.agents}
        ...
        for agent_name in env.env.agents:
            ...
            print(f"Action Reasoning: {episode_dict[step][agent_name]['reasoning']}")
            # print("Objective Reasoning: ",episode_dict[step][agent_name]['objective_reasoning'])
            # print("Plan Reasoning: ",episode_dict[step][agent_name]['plan_reasoning'])
            print("Plan :", episode_dict[step][agent_name]['plan'])
        ...
        episode_dict[step]["stats"]["rewards"] = cum_rewards
        print(f"\n=== Environment Stats ===")
        print(f"Score: {stats['score']}")
        print(f"Rewards: {cum_rewards}")
...
def load_joint_policy(run_path: str):
    spec = importlib.util.spec_from_file_location(
        "joint_policy",                        # module name (arbitrary)
        os.path.join(run_path, "joint_policy_test_gemma4.py")
    )
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module.CraftaxJointPolicy
...
def main(config: DictConfig) -> None:
    ...
    run_path = os.path.join(original_cwd, "runs")
    ...
    config["run_path"] = os.path.join(run_path, run_name)
...
# env_name = "Craftax-Coop-Symbolic"
    ...
    agents = {agent_name: ExecAgent(agent_name, LLM_client, TaskExecutionPromptBuilder(config)) for agent_name in env.agents}


    for aid, agent_name in enumerate(env.agents):
        
        system_prompt = get_task_selection_system_prompt(aid)
        agents[agent_name].prompt_builder.update_instruction_prompt(system_prompt)

    episode_dict, img_lst = run_task_execution(env, agents, config)
